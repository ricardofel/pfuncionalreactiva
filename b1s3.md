...
SOLUCION:
```scala
val entropiaGasIdeal = (n: Double, R: Double, V1: Double, V2: Double, Cv: Double, T1: Double, T2: Double) =>
...
SOLUCION:
```scala
val velocidadEscape = (G: Double, M: Double, r: Double) =>
  math.sqrt((2 * G * M) / r)
```
...
SOLUCION:
```scala
val solucionCuadratica = (a: Double, b: Double, c: Double) => {
  val discriminante = math.sqrt(b * b - 4 * a * c)  // Calculamos la raíz cuadrada del discriminante
  val x1 = (-b + discriminante) / (2 * a)           // Solución con el signo +
  val x2 = (-b - discriminante) / (2 * a)           // Solución con el signo -
  (x1, x2)                                          // Retornamos ambas soluciones en una tupla
}
```
...
```
SOLUCION:
La función cabecera devuelve otra función que toma un String como parámetro y devuelve un String. 
Entonces, el tipo de cabecera sería:
```scala
...
```scala
//Funcion principal
def calcArea(f:(Int, Int) => Double,a: Int, b: Int) = {
  val result = f(a,b)
  result
} 
//Funciones de las areas
def areaTriangulo(a: Int, b: Int) = (a*b)/2 //AREA TRIAGULO
def areaRectangulo(a: Int, b: Int) = a*b //AREA RECTANGULO
def areaRombo(D: Int, d: Int) = (D*d)/2 //AREA ROMBO
def areaPoligonoRegular(p: Int, a: Int) = (p*a)/2 //AREA POLIGONO REGULAR

//COMPROBAR
calcArea(areaTriangulo, 4,4)
calcArea(areaRectangulo, 4,4)
calcArea(areaRombo, 4,4)
calcArea(areaPoligonoRegular, 4,4)
```
### Calificación:
